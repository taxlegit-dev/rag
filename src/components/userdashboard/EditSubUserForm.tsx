...
export default function EditSubUserForm({
  subUser,
  departments,
  onSuccess,
  onCancel,
}: EditSubUserFormProps) {
  const [formData, setFormData] = useState({
    name: subUser.name,
    contactNumber: subUser.contactNumber,
    departmentId: subUser.department.id,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || "Failed to update sub-user");
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-slate-800 rounded-lg p-6 border border-purple-500/20 w-full max-w-md mx-4">
        <h3 className="text-lg font-semibold text-white mb-4">Edit Sub-user</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex space-x-3">
            ...
            >
              {loading ? "Updating..." : "Update Sub-user"}
            </button>
            <button
              type="button"
              onClick={onCancel}
